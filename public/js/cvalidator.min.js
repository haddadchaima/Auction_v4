...
(function($){
    ...
    $.fn.cValidate = function(options){
        ...
        var cRules = {
            ...
            time: function(thisField, value, key, parameter, name, type, isBetween, form){
                if(value == ''){return false;}
            ...
            dateTime: function(thisField, value, key, parameter, name, type, isBetween, form){
                if(value == ''){return false;}
                var timeParts = value.split(' ');
                if(
                    timeParts.length<2 ||
                    !checkdate(timeParts[0]) ||
                    this.time(thisField, timeParts[1], key, parameter, name, type, isBetween, form)
        ...
        cValidate.each(function(){
            ...
            console.log("ajaxMethod : ",ajaxMethod);
            if(ajaxMethod && ajaxMethod.toUpperCase() === 'POST'){
				ajaxMethod = 'POST';
	     	}else if(ajaxMethod && ajaxMethod.toUpperCase() === 'PUT'){
				 ajaxMethod = 'PUT';
				}
				else
				{
