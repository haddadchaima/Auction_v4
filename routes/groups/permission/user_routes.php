...
//   User Verifications Controllers
    ...
    Route::get('profile/verification-avec-cin', 'VerificationController@verificationWithIdIndex')->name('profile-verification-with-id.index');

    Route::get('profile/verification/upload-cin', 'VerificationController@createWithId')->name('profile-verification-with-id.create');
    Route::post('profile/verification/upload-cin', 'VerificationController@verificationWithIdStore')->name('profile-verification-with-id.store');
...
//   Seller Verifications Controllers
    Route::get('gerer-magasin/verification', 'SellerVerificationController@Index')->name('seller-verification.index');

    Route::get('gerer-magasin/verification/upload-cin', 'SellerVerificationController@createWithId')->name('seller-verification-with-id.create');
    Route::post('gerer-magasin/verification/upload-cin', 'SellerVerificationController@verificationWithIdStore')->name('seller-verification-with-id.store');

    Route::get('gerer-magasin/verification/upload-address', 'SellerVerificationController@createWithAddress')->name('seller-verification-with-address.create');
    Route::post('gerer-magasin/verification/upload-address', 'SellerVerificationController@verificationWithAddressStore')->name('seller-verification-with-address.store');

//    Buyer Profile controllers
    Route::get('profile','UserProfileController@userIndex')->name('user-profile.index');
    Route::get('profile/remporter','UserProfileController@userWonAuctionIndex')->name('user-profile-won-auction.index');

    Route::get('profile/gerer-profile','UserProfileController@index')->name('user-manage-profile.index');

    Route::get('profile/editer', 'UserProfileController@edit')->name('user-profile.edit');
    Route::put('profile/update', 'UserProfileController@update')->name('user-profile.update');
    Route::get('profile/changer-password', 'UserProfileController@changePassword')->name('user-profile.change-password');
    Route::put('profile/changer-password/update', 'UserProfileController@updatePassword')->name('user-profile.update-password');
...
//    Seller Profile Controllers
    ...
    Route::get('magasin/{status?}','Seller\StoreController@sellerIndex')->name('seller-profile.index');
    Route::get('profile/{id}/vendeur/{status?}','Seller\StoreController@show')->name('seller-profile.show');

//    Store Management Controllers
    Route::get('gerer-magasin', 'Seller\StoreManagementController@index')->name('store-management.index');
    Route::resource('gerer-magasin/address', 'AddressController')->names('address')->parameter('address', 'id');


    Route::post('pays', 'AddressController@getStateByCountry')->name('get-state.index');
    Route::put('gerer-magasin/address/{id}', 'AddressController@makeAddressDefault')->name('change-address-status.update');
...
//    Auction
    Route::get('vendeur/enchere/creer', 'AuctionController@create')->name('auction.create');
    Route::post('vendeur/enchere/magasin', 'AuctionController@store')->name('auction.store');

    Route::put('livraison-status/enchere/{id}', 'AuctionController@updateShippingStatusFromSeller')->name('update-shipping-status.update');
    Route::put('livraison-status/enchere/{id}/livraison-status', 'AuctionController@updateShippingStatusFromUser')->name('update-shipping-status-user.update');
    Route::get('profile/enchere/{id}/livraison-description', 'AuctionController@shippingDescriptionCreate')->name('shipping-description.create');
    Route::post('profile/enchere/{id}/livraison-description', 'AuctionController@shippingDescriptionUpdate')->name('shipping-description.update');

//    Bidding controller
    Route::post('offre/{auctionId}', 'BidController@store')->name('bid.store');
...
//    Notification controller
    ...
    Route::get('profile/{id}/lus','UserNotificationController@markAsRead')->name('notification.mark-as-read');
    Route::get('profile/{id}/nonlus','UserNotificationController@markAsUnread')->name('notification.mark-as-unread');

    //dispute controller
    Route::resource('pleinte', 'DisputeController')->names('dispute')->parameter('dispute', 'id')->only(['create', 'store']);
    Route::get('specific-pleinte/{dispute_type}/{ref_id}' , 'DisputeController@specific')->name('disputes.specific');
    Route::get('pleinte/{type?}' , 'DisputeController@index')->name('dispute.index');

    Route::get('user-disputes/{id}/lus', 'DisputeController@markAsRead')->name('dispute.mark-as-read');
    Route::get('user-disputes/{id}/nonlus', 'DisputeController@markAsUnread')->name('dispute.mark-as-unread');
/*********************ENGLAIS***********************/
//   User Verifications Controllers
    Route::get('profile/verification', 'VerificationController@Index')->name('profile-verification.index');
...
Route::namespace('Transaction')->group(function () {
    ...
    Route::get('profile/depot/{id}/creer', 'WalletTransactionController@create')->name('wallet-deposit.create');
    Route::post('profile/depot/{id}/magasin', 'WalletTransactionController@store')->name('wallet-deposit.store');
    Route::get('profile/depot-historique/{payment_status?}', 'WalletTransactionController@index')->name('deposit.index');

    Route::get('profile/devise', 'WalletController@index')->name('user-currency.index');
    ...
    //to withdrawal money
    Route::post('profile/retrait/{id}/magasin', 'WithdrawalController@store')->name('withdrawal-form.store');
    Route::get('profile/retrait/{id}/create', 'WithdrawalController@create')->name('withdrawal-form');
    Route::get('profile/retrait-historique/{payment_status?}', 'WithdrawalController@index')->name('withdrawal.index');
/************************ANGLAIS******************/
    //wallet history route
    Route::get('payment-method/paypal/completed', 'WalletTransactionController@paypalReturnUrl')->name('paypal.return-url');
    Route::get('payment-method/paypal/cancel', 'WalletTransactionController@paypalCancelUrl')->name('paypal.cancel-url');
