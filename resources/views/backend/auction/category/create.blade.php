        ...
        <div class="col-lg-12">
            @component('components.card',['type' => 'info'])
                ...
                <div class="row justify-content-center">
                    <div class="col-md-5">
                        {{ Form::open(['route' => 'category.store', 'class' => 'cvalidate']) }}
                        @method('post')
                        @basekey

                        <div class="form-group">
                                <label class="{{fake_field('form-control-label mb-3')}}" for="{{fake_field('category-name')}}">Category Name :</label>
                                {{Form::text(fake_field('name'), old('name'), ['class' => form_validation($errors, 'name form-control'), 'id' => fake_field('category-name'), 'data-cval-name' => 'Category name required', 'data-cval-rules' => 'required|min:3' ] )}}
