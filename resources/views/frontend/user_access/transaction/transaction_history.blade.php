                ...
                @component('components.card',['type' => 'info', 'class' => 'card text-right mt-4'])
                    ...
                    @component('components.table',['class' => 'cm-data-table '])
                        ...
                        @foreach($list['items'] as $earning)
                                <tr>
                                    <td class="text-left"> <span class="font-weight-bold">{{user_transaction_type($earning->journal)}}</span></td>
                                    <td class="text-center">{{$earning->ref_id}}</td>
                                    <td class="text-right font-weight-bold color-default">{{$earning->amount}} {{settings('default_currencies')}}</td>
                        ...
                        <div class="col-sm-12 col-md-6">
                            <table class="table text-right my-5">
                                ...
                                <tr>
                                    ...
                                    <th class="border-bottom-0"> {{$previousTotal}}  {{settings('default_currencies')}}</th>
                                </tr>
                                </thead>
                                <tbody>
                                @forelse($currentCalculation as $key=>$val)
                                    <tr>
                                        ...
                                        <td>:</td>
                                        <td> {{ $currentCalculation[$key] }}  {{settings('default_currencies')}}</td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td>Others</td>
                                        <td>:</td>
                                        <td>0.00  {{settings('default_currencies')}}</td>
                                    </tr>
                                @endforelse
                                </tbody>
                                <tfoot class="border-bottom">
                                    ...
                                    <th class="bg-light"> {{ !empty($currentCalculation) ? bcadd(array_sum($currentCalculation),$previousTotal) : $previousTotal }}  {{settings('default_currencies')}}</th>
                                </tfoot>
                            </table>
                        </div>
                    </div>
                        ...
                        <div class="card-body text-center">
                            <div class="row">
                                <div class="col-sm-4 clo-md-3 transaction-list">
                                    <a class="text-capitalize border my-1 w-100 py-2 px-3 d-inline-block {{empty($transactionType) ? 'active' : '' }}" href="{{route('transaction-history')}}">{{__('all transactions')}}</a>
                                </div>
                                @foreach(user_transaction_type() as $key=>$val)
                                    <div class="col-sm-4 clo-md-3 transaction-list">
                                        <a class="text-capitalize border my-1 w-100 py-2 px-3 d-inline-block {{$transactionType==$key ? 'active' : ''}}" href="{{route('transaction-history', $key)}}">{{$val}}</a>
                                    </div>
                                @endforeach
                            </div>
                        </div>
